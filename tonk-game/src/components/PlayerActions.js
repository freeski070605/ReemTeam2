...
const PlayerActions = ({ 
    ...
    return (
        <div className="player-actions">
            <button 
                className={`action-button drop ${!hasDrawnCard && canDrop ? 'active' : ''}`}
                disabled={!isActive || hasDrawnCard || !canDrop}
                onClick={handleDropAction}
            >
                Drop
            </button>
            <button 
                className={`action-button spread ${hasDrawnCard && canSpread ? 'active' : ''}`}
                disabled={!canSpread}
